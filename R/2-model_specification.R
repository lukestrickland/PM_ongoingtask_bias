#NEW Model: t0 can vary by day
#to incoprorate possible practice effects
# (reviewer request)
...
p_prior_top_t0 <-   prior.p.dmc(
  p1=c(t0.one=1,t0.two=1, p_vector_top_t0[-(1:2)]),
  dists=  c(rep("beta", 2), rep("tnorm", length(p_vector_top_t0)-2)),
  p2= c(rep(1,2),1,1,rep(1,12),rep(2,41)),
  lower=c(rep(0.1,2),0,0,rep(0,12), rep(NA, 41)),
  upper=c(rep(1,2),10,rep(NA, length(p_vector_top_t0)-3))
)

dm_top_t0 <- data.model.dmc(okdats,model_top_t0)

samples_top_t0 <- h.samples.dmc(nmc = 180,p_prior_top_t0,dm_top_t0, thin=20)
...
#to source)
save(samples_top_t0, file="samples_top_t0.RData")
